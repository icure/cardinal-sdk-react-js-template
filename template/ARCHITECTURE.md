...
A guide for developers extending this app. Explains *why* each layer is shaped the way it is, *what already works*, and *what you still have to build* before this is a real product.

For a quick start (env vars, `yarn start`), see [`README.md`](./README.md). This file is the architectural tour.
...
### 2.1 Signup / first login (process-based)
...
### 2.2 Returning login (credentials-based)
...
## 3. Why `redux-persist` whitelists only `app`
...
## 5. Encryption model (CryptoStrategies, why it matters)
...
The Cardinal SDK accepts `CaptchaOptions.Kerberus.Computed({ solution })`. The browser:
...
The `KerberusCaptcha` component (`src/components/authentication/KerberusCaptcha/`) encapsulates fetch + resolve + progress reporting and is mounted inside both `LoginForm` and `SignupForm`. It auto-resolves on mount and renders an Antd `<Progress>` while computing; submit is disabled until the `Solution` arrives. If the user takes too long to submit, you can force a re-resolve by bumping the optional `refreshCounter` prop.

There is no extra site key — Kerberus challenges are issued by the Cardinal message gateway, identified by your `REACT_APP_EXTERNAL_SERVICES_SPEC_ID`.

## 7. Where to add a new Cardinal-backed feature
...
**Never re-instantiate `CardinalSdk` outside `auth.api.ts`. Always go through `cardinalApi(getState)` → the cached instance.** The SDK is expensive to construct (it does crypto bootstrap, hierarchy walks, etc.) and there's no benefit to creating a second instance per query.

## 8. Implemented vs. to-build
...
- Antd theming (`src/style/antd/antdTheme.ts`), Less compilation pipeline, committed `.css` next to source `.less`.

### You'll need to build for a real product

- **Multi-group / environment selection UI.** This app auto-picks the first group in `groupSelector` and warns to console. Real products with multiple environments need a selector modal that resolves the `groupSelector` promise after user input.
...
- **Parent-HCP key bootstrap** if your product has parent organisations whose keys need recovery alongside the user's own. Will require extending `TemplateCryptoStrategies` with a parent recovery branch.
- **Domain features.** `practitionerApi.ts` is the only example. Real apps need `Patient`, `HealthElement`, `Contact`/`Service`, `Document`, `Message`, `Agenda`/`CalendarItem`, etc. — one file per domain in `src/core/api/`.
- **Internationalisation.** No i18n library is wired. `react-i18next` slots in cleanly.
...
- **CI.** Add typecheck/build/test on PR.

## 9. Conventions

- **Code style:** ESLint + Prettier from `.eslintrc` and `.prettierrc`. No semicolons, single quotes, trailing commas, `printWidth: 180`, 2-space indent.
- **Component layout:** `Component/index.tsx` with a sibling `index.less`; the `start` script's `less-watch-compiler` produces `index.css` and `index.css.map` next to it.
- **Typed Redux:** import `useAppDispatch` / `useAppSelector` from `src/core/hooks.ts`, never the raw `react-redux` versions.
- **SDK access:** always go through `cardinalApi(getState)` → `apiCache`. Never re-instantiate `CardinalSdk` outside `auth.api.ts`.
- **Adding new state:** if it's serializable and survives reload (e.g. user preferences), put it in the `app` slice and add to the `whitelist` in `src/core/app/index.ts`. Otherwise add it to the `cardinalApi` slice or create a new slice.
...
- [Create React App](https://create-react-app.dev/) for anything CRA-specific (testing, environment variables, code splitting, deployment).
